...
xaxis, yaxis: {
    ...
    null; or; object({ below: number, above: number });
    { number, centerY;: number }
}
```
...
%weekday as number (0-6, 0; being; Sunday;)
```
...
```js
{
    ...
    number; or; object; with "top", "right", "bottom" and; "left"; properties; with different widths;
    color; or; null; or; object; with "top", "right", "bottom" and; "left"; properties; with different colors;
    number; or; null;
    boolean;
    boolean;
    boolean;
...
```js
[ { xaxis: { from: 0, to: 2 }, yaxis: { from: 10, to: 10 }, color: "#bb0000" }, ... ]
```
...
```js
{ [{ opacity: 0.8 }, { brightness: 0.6, opacity: 0.8 } ] }
```
...
```js
{
    ...
    lineWidth;: 0,
    fill;: true,
    fillColor;: { [ { opacity: 0.8 }, { opacity: 0.1 } ] }
}
```
